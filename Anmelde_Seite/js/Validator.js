class Validator {
	...
	validate(e) {
		...
		if(elFields[fieldName].required) {
			if(fieldValue === '') {
				this.errors[fieldName].push('Feld ist leer');
			}
		}

		if(elFields[fieldName].email) {
			if(!this.validateEmail(fieldValue)) {
				this.errors[fieldName].push('ungültige email adresse');
			}
		}

		if(fieldValue.length < elFields[fieldName].minlength || fieldValue.length > elFields[fieldName].maxlength) {
			this.errors[fieldName].push(`Feld muss minimal ${elFields[fieldName].minlength} oder maximal ${elFields[fieldName].maxlength} Zeichen haben`);
		}
		...
		if(elFields[fieldName].matching) {
			...
			if(fieldValue !== matchingEl.value) {
				this.errors[fieldName].push('Passwort ist nicht gleich');
			}
